...
- **Surfpool never relays transactions to mainnet** — verified in the v1.5.0 source (`sendTransaction` only reaches the embedded LiteSVM; the remote client makes read-only calls) and cross-checked on mainnet (none of the 969 local signatures or 40 created accounts exist there). Local Surfpool runs are therefore safe under the "no mainnet transactions" rule. Note: the release binary makes one read-only version check to `cloud.txtx.run` at startup.
